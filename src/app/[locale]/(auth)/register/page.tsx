  ...
  return (
    <div style={{ width: '100%', maxWidth: 440 }}>
        ...
        <div style={{ border: `1px solid ${ACC}`, background: '#FBFAF6', padding: '20px 24px', marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            <div>
              ...
              <span style={{ marginLeft: 10, fontSize: 11, color: MUTE, letterSpacing: '0.08em' }}>{t('selected')}</span>
            </div>
      ...
      <div style={{ position: 'relative', border: `1px solid ${RULE}`, background: '#FBFAF6', padding: '40px 36px' }}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <p style={{ fontSize: 11.5, textAlign: 'center', color: MUTE }}>
            {t('termsText')}{' '}
            <Link href="/terms" style={{ color: INK, borderBottom: `1px solid ${RULE}`, paddingBottom: 1 }}>{t('termsLink')}</Link>{' '}
            &{' '}
            <Link href="/privacy" style={{ color: INK, borderBottom: `1px solid ${RULE}`, paddingBottom: 1 }}>{t('privacyLink')}</Link>.
          </p>
        </form>
      </div>
      ...
      <div style={{ marginTop: 24, textAlign: 'center', fontSize: 13, color: MUTE }}>
        {t('hasAccount')}{' '}
        <Link href="/login" style={{ color: INK, borderBottom: `1px solid ${RULE}`, paddingBottom: 1 }}>
          {t('signIn')}
        </Link>
      </div>
    </div>
